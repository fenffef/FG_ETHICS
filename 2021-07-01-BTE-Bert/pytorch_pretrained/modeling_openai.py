...
class OpenAIGPTConfig(object):
    """Configuration class.txt to store the configuration of a `OpenAIGPTModel`.
    """
...
class OpenAIGPTPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, OpenAIGPTConfig):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class.txt `OpenAIGPTConfig`. "
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    Params:
        config: a OpenAIGPTConfig class.txt instance with the configuration to build a new model
